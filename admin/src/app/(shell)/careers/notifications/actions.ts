...
/**
 * The writes behind the Careers → Notifications tab.
 *
 ...
 * event; these post to `/admin/careers/notifications/`, which is Owner + Manager and
 * reaches exactly one. Sharing the module would mean one `BASE` constant deciding which
 * permission boundary a click lands on, and the day somebody parameterised it wrongly
 * the mistake would be invisible on both screens. Two short files, two fixed URLs.
 *
 * `event` IS NOT SENT. The backend pins it (`CareersRecipientSerializer` makes the field
 * read-only), so a value here would be decoration that looks load-bearing. The shared
...
const BASE = "/admin/careers/notifications/";
...
const DENIED = "Your role does not include changing who is emailed about applications.";
