 ...
 * after the combo is live and the URL is wrong.
 */
import { useActionState, useState } from "react";
import Link from "next/link";
import { slugFollowsName, slugify } from "@/lib/slugify";
import type { CreateState } from "@/app/(shell)/combos/new/actions";
...
export function NewComboForm({
  ...
  // Seeded from the returned values so a rejected form does not start auto-rewriting a
  // slug the person had chosen.
  ...
  return (
    <form action={formAction} className="max-w-lg">
      ...
      <div className="space-y-4">
        <label className="block text-xs text-muted">
          ...
          <input
            ...
            placeholder="Radiant Glow Combo"
            autoFocus
        ...
        <label className="block text-xs text-muted">
          ...
          <p className="mt-1 text-xs text-muted">
            The storefront URL: <span className="font-mono">/combo/{slug || "…"}</span>
          </p>
      ...
      <p className="mt-4 rounded-[var(--radius-card)] border border-line bg-surface p-3 text-sm text-muted">
        It is created as a <strong>draft</strong>. You pick the products, the markets and
        the price on the next screen, and it goes live when you set it to Active.
      </p>
      ...
      <div className="mt-4 flex items-center gap-2">
        ...
        >
          {pending ? "Creating…" : "Create combo"}
        </button>
        <Link href="/combos" className="text-sm text-muted underline-offset-2 hover:underline">
          Cancel
        </Link>
      </div>
    </form>
