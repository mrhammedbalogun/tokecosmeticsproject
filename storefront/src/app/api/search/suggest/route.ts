...
export async function GET(req: Request) {
  ...
  // Public endpoint: cap length before forwarding (defense-in-depth vs oversized q).
  ...
  try {
    // 2 s upstream timeout so a hung backend can't stall a keystroke-latency call.
