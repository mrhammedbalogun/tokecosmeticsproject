    ...
    <section aria-label="TikTok favourites" className="wrap py-8">
      <FadeUp>
        <div className="grid gap-4 lg:grid-cols-[1fr_1.1fr]">
          ...
          <ArtworkOrOverlay banner={banner} />
        </div>
      </FadeUp>
    </section>
  );
...
/** The panel beside the products. A finished piece already carries its words, so the
 * shop writes none of its own and the whole panel becomes the link; `min-h-[420px]`
 * stays on the box either way, because `TileMedia` is absolutely positioned and the copy
 * block is what would otherwise give this tile its height. */
function ArtworkOrOverlay({ banner }: { banner?: CmsBanner | null }) {
  const heading = banner?.title || "TikTok Made Me Try It";
  const href = banner?.cta_url || "/products?collection=best-sellers";
  const box = "relative flex min-h-[420px] items-end overflow-hidden rounded-[var(--radius-card)]";
  const media = (
  ...
  return (
    <div className={box}>
      ...
      <div className="relative p-10">
        <h2 className="font-display text-3xl italic text-surface md:text-4xl">{heading}</h2>
