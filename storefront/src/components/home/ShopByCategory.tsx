      ...
      <FadeUp>
        ...
        <div className="mt-8 grid grid-cols-2 gap-4 lg:grid-cols-4">
          {tiles.map((tile) => {
            ...
            // The pill IS this tile's accessible name — every tile image renders
            // `alt=""` — so a finished piece keeps it as `sr-only` rather than dropping
            // it, which would leave a link a screen reader announces as nothing.
              ...
              >
                ...
                <span
                  className={
                    artwork
                      ? "sr-only"
                      : "relative rounded-full border border-surface/60 bg-black/35 px-6 py-2.5 text-xs uppercase tracking-[0.14em] text-surface backdrop-blur-sm transition-colors group-hover:border-accent group-hover:bg-accent"
                  }
                >
                  {label}
                </span>
