 ...
 *
 * The heading states one rate when every combo in the row agrees on it and "up to" the
 * best one when they differ (see `savingClaim`) — it keeps selling the discount either
 * way, and each card's badge carries its own exact figure.
 *
...
export function ComboRow({ combos }: { combos: ComboCardData[] }) {
  ...
  const claim = savingClaim(shown);
  const title = claim
    ? `Save ${claim.upTo ? "up to " : ""}${formatPercent(claim.percent)}% when you buy the set`
