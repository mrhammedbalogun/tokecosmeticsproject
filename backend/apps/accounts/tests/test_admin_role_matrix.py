...
MATRIX: list[Row] = [
    ...
    Row("ReferrerListView", "get", "/api/v1/admin/referrers/",
        {"Owner", "Manager", "Support"}),
    Row("ReferrerAdjustmentsView", "get", "/api/v1/admin/referrers/999999/adjustments/",
        {"Owner", "Manager", "Support"}),
    Row("BlockReferrerView", "post", "/api/v1/admin/referrers/999999/block/",
        _MANAGERS, body={"blocked": True, "reason": "abuse"}),
    ...
    # The media library feeds the banner tiles, so it sits with them.
    Row("MediaAssetAdminViewSet", "get", "/api/v1/admin/media/", _MANAGERS),
...
@pytest.mark.parametrize(
    "row",
    MATRIX + [CANCEL_ROW, DELETE_PRODUCT_ROW, DELETE_VARIANT_ROW],
    ids=_ids(MATRIX + [CANCEL_ROW, DELETE_PRODUCT_ROW, DELETE_VARIANT_ROW]),
)
...
def test_the_matrix_agrees_with_the_scope_table():
    ...
    """
    for row in MATRIX + [CANCEL_ROW, DELETE_PRODUCT_ROW, DELETE_VARIANT_ROW]:
