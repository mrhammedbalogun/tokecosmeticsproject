"""The careers tab's write shape: the general recipient serializer with its event nailed
down.
...
`apps/careers/notification_views.py` is built on. `event` is therefore read-only here
and supplied by `create()`.
"""
...
class CareersRecipientSerializer(NotificationRecipientSerializer):
