...
- Passwords: **three** formats, not two — `$wp$` (bcrypt over an HMAC-SHA384 pre-hash),
  phpass `$P$`/`$H$`, and plain `$2y$`. `apps/accounts/hashers.py` covers all three and is
  pinned against hashes generated by the VPS's own PHP 8.1.34. NG current is 100% `$wp$`;
  the phpass path serves 86 users (84 NG-old, 2 intl).
